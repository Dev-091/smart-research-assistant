...
def _handle_sidebar_actions(sidebar_state, document_manager):
    ...
    if sidebar_state["build_index"] or sidebar_state["rebuild_index"]:
        ...
        try:
            with st.spinner(f"{action_label} knowledge base..."):
                build_summary = document_manager.build_knowledge_base()
                reload_rag_service()
...
def main():
    ...
    sidebar_state = render_sidebar(document_manager)
    _handle_sidebar_actions(sidebar_state, document_manager)
